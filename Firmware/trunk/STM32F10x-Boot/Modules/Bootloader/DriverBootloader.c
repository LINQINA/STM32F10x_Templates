...
int8_t cFirmwareUpdate(FirmwarePortInfoType *pTarget, FirmwarePortInfoType *pSource)
{
    ...
    printf("update source addr: 0x%08X, target addr: 0x%08X, file size: %d\r\n", pSource->address, pTarget->address, (int)(pSource->length));
...
int8_t cFirmwareJumpFactoryBootloader(void)
{
    ...
    const uint32_t uiBootAddr = 0x1FFFF000;
    typedef void (*pFunction)(void);
    uint32_t jumpAddress = *(__IO uint32_t*)(uiBootAddr + 4);

    pFunction JumpToBoot = (pFunction)jumpAddress;
    __set_MSP(*(__IO uint32_t*)uiBootAddr); // 设置栈顶指针
...
int8_t cFirmwareUpdateForce(FirmwarePortInfoType *pTarget, FirmwarePortInfoType *pSource)
{
    uint32_t uiSourceAdddrNow = 0, uiTargetAdddrNow = 0, uiCRCValue = 0;
    int32_t iLength = 0, iLengthNow = 0;
    int8_t cError = 0;
...
int8_t cFirmwareUpdateALLForce(void)
{
    ...
    printf("Update All Force Start \r\n");

    /* Bootloader 强制恢复 */
    cError = cFirmwareUpdateForce(&st_typeFirmwareInfo.bootloader, &st_typeFirmwareInfo.bootloaderOut);
    if (cError != 0)
    {
        printf("Force Update Bootloader Fail \r\n");
        return 1;
    }

    /* APP 强制恢复 */
    cError = cFirmwareUpdateForce(&st_typeFirmwareInfo.app, &st_typeFirmwareInfo.appOut);
    if (cError != 0)
    {
        printf("Force Update APP Fail \r\n");
        return 2;
    }

    printf("Update All Force Success \r\n");
