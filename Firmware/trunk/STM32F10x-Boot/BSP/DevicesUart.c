...
void vUart2Init(void)
{
    ...
    __HAL_UART_ENABLE_IT(&g_uart2_handle, UART_IT_RXNE); //使能串口1空闲中断
    __HAL_UART_ENABLE_IT(&g_uart2_handle, UART_IT_IDLE); //使能串口1空闲中断
    __HAL_UART_ENABLE_IT(&g_uart2_handle, UART_IT_ERR); //使能串口1错误中断
...
{
    while ((UART_LOG->SR & 0X40) == 0);     /* 等待上一个字符发送完成 */

    UART_LOG->DR = (uint8_t)ch;             /* 将要发送的字符 ch 写入到DR寄存器 */
    return ch;
}
#endif
