...
void vSPI2Init(void)
{
    ...
    spi2_init_struct.Init.Mode = SPI_MODE_MASTER;                            /* 主机模式 */
    spi2_init_struct.Init.Direction = SPI_DIRECTION_2LINES;                  /* 全双工模式 */
    spi2_init_struct.Init.DataSize = SPI_DATASIZE_8BIT;                       /* 字节格式传输 */
    spi2_init_struct.Init.CLKPolarity = SPI_POLARITY_HIGH;                   /* 空闲时钟为高电平 */
    spi2_init_struct.Init.CLKPhase = SPI_PHASE_2EDGE;                        /* 第二个时钟沿采样 */
    spi2_init_struct.Init.NSS = SPI_NSS_SOFT;                                /* 软件管理片选 */
    spi2_init_struct.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_16;      /* 分频系数 16 */
    spi2_init_struct.Init.FirstBit = SPI_FIRSTBIT_MSB;                       /* 高位在前 */
    spi2_init_struct.Init.TIMode = SPI_TIMODE_DISABLE;                       /* 禁用 TI 模式 */
    spi2_init_struct.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;       /* 禁用 CRC 计算 */
