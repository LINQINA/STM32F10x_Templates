...
/*
 * Return:      按键状态̬
 * Parameters:  Value_new:新按键键值ֵ; Value_old:旧按键键值ֵ;
 * Description: 按键状态获取
 */
static KeyStateEnum enumKeyStateGet(uint32_t uiNewValue, uint32_t uiOldValue)
{
    if(uiNewValue == uiOldValue)                            /* 当前按键值与上次按键值相同 */
        return keyEqual;
        else if((uiNewValue & uiOldValue) == uiOldValue)    /* 当前按键值与上次按键值基础上增加了新按键 */
        return keyAdd;
    else if((uiNewValue | uiOldValue) == uiOldValue)        /* 当前按键值与上次按键值基础上减少了旧按键 */
        return keyCut;
    else                                                    /* 当前按键值与上次按键值基础上减少了旧按键又增加了新按键 */
        return keyAddAndCut;
}
