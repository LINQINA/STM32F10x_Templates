...
void vCan1Init(void)
{
    ...
    g_can1_handler.Init.Mode = CAN_MODE_NORMAL;        /* 正常模式 */
    ...
    g_can1_handler.Init.SyncJumpWidth    = CAN_SJW_1TQ;  /* 同步跳转宽度 */
    ...
    // 设置接收的ID列表
    can_filterconfig.FilterIdHigh = (0x350 << 5);             /* 标准帧 ID 左移 5 位存入高 16 位 */
    can_filterconfig.FilterIdLow = 0x0000;                    /* 低 16 位 */
    can_filterconfig.FilterMaskIdHigh = 0x0000;               /* 列表模式不需要，保持默认值 */
    can_filterconfig.FilterMaskIdLow = 0x0000;                /* 列表模式不需要，保持默认值 */
